...
public class CookieUtils {
    ...
    public static void destroyLoginCookie(HttpServletResponse response) {
        ...
        Cookie cookieAct = new Cookie(Contants.COOKIE_NAME_ACT, "1");
        cookieAct.setMaxAge(0);
        response.addCookie(cookieAct);
        Cookie cookiePwd = new Cookie(Contants.COOKIE_NAME_PWD, "1");
        cookiePwd.setMaxAge(0);
        response.addCookie(cookiePwd);
        Cookie cookieUserId = new Cookie(Contants.COOKIE_NAME_ID, "1");
    ...
    public static void createLoginCookie(User user, HttpServletResponse response) {
        Cookie cookieAct = new Cookie(Contants.COOKIE_NAME_ACT, user.getLoginAct());
        cookieAct.setMaxAge(Contants.COOKIE_MAX_ALIVE_TIME);
        response.addCookie(cookieAct);
        Cookie cookiePwd = new Cookie(Contants.COOKIE_NAME_PWD, user.getLoginPwd());
        cookiePwd.setMaxAge(Contants.COOKIE_MAX_ALIVE_TIME);
        response.addCookie(cookiePwd);
        // 新建一个存放用户id的cookie，代表该用户的唯一标识，用于登录时用户判断
        Cookie cookieUserId = new Cookie(Contants.COOKIE_NAME_ID, user.getId());
        cookieUserId.setMaxAge(Contants.COOKIE_MAX_ALIVE_TIME);
        response.addCookie(cookieUserId);
    }
