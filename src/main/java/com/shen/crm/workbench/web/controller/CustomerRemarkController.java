...
import com.shen.crm.workbench.service.CustomerRemarkService;
...
public class CustomerRemarkController {
    ...
    public Object saveCreateCustomerRemark(CustomerRemark customerRemark, HttpSession session) {
        ...
        customerRemark.setCreateTime(DateUtils.formateDate(new Date()));
        customerRemark.setEditFlag(Contants.REMARK_EDIT_FLAG_FALSE);
        ...
        try {
            ...
            if (res > 0) { // 插入成功
                ...
                returnObject.setRetData(customerRemark); // 将备注也传到前端响应到页面
    ...
    public Object saveEditCustomerRemark(CustomerRemark customerRemark, HttpSession session) {
        User user = (User) session.getAttribute(Contants.SESSION_USER);
        // 封装参数
        customerRemark.setEditFlag(Contants.REMARK_EDIT_FLAG_TRUE);
